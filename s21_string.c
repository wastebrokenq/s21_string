...
static const char *s21_errors[] = {
    ...
    "Try again",                       // 11 (EAGAIN/EWOULDBLOCK)
    ...
    "Resource deadlock would occur",   // 35 (EDEADLK/EDEADLOCK)
    "File name too long",              // 36 (ENAMETOOLONG)
    "No record locks available",       // 37 (ENOLCK)
    "Invalid system call number",      // 38 (ENOSYS)
    "Directory not empty",             // 39 (ENOTEMPTY)
    "Too many symbolic links encountered", // 40 (ELOOP)
    "Operation would block",           // 41 (EWOULDBLOCK, псевдоним EAGAIN)
    ...
    "Resource deadlock would occur",   // 58 (EDEADLOCK, псевдоним EDEADLK)
